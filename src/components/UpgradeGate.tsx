...
// Maps feature → the minimum plan tier that unlocks it
const FEATURE_TIER: Record<string, string> = {
  documentGeneration: 'Basic',
  philanthropy: 'Standard',
  assets: 'Standard',
  advancedFinances: 'Standard',
  ministries: 'Standard',
  importData: 'Standard',
  bloodbank: 'Premium',
  youth: 'Premium',
...
export default function UpgradeGate({ feature, featureLabel, compact = false }: UpgradeGateProps) {
  ...
  const tier = FEATURE_TIER[feature] ?? 'Premium'
  const highlights = TIER_HIGHLIGHTS[tier] ?? TIER_HIGHLIGHTS['Premium']

  if (compact) {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-700 bg-[var(--warning-light)] border border-amber-200 px-2 py-0.5 rounded-full">
    ...
    <div className="flex flex-col items-center justify-center min-h-[420px] text-center px-4 py-16 space-y-6">
      ...
      <div className="space-y-2 max-w-md">
        ...
        <p className="text-[var(--text-muted)] text-sm leading-relaxed">
          ...
          <span className="font-semibold text-[var(--brand)]">{tier} πακέτο</span> της
