...
export default function ChangelogPage() {
 ...
 <div className="bg-[var(--surface)]/20 p-2 rounded-xl"><Zap className="text-yellow-300 w-6 h-6"/></div>
 </div>
 ...
 <div key={idx} className="bg-[var(--surface)] border text-left border-[var(--border)] rounded-2xl shadow-sm overflow-hidden flex flex-col md:flex-row">
 <div className="bg-[var(--background)] md:w-1/4 p-6 border-b md:border-b-0 md:border-r border-[var(--border)] flex flex-col justify-center">
 <div className="text-sm font-extrabold text-blue-600">{entry.version}</div>
 <div className="text-xs text-[var(--text-muted)] font-mono mt-1">{entry.date}</div>
 <span className="mt-3 inline-block bg-slate-200 text-[var(--text-secondary)] text-[10px] font-bold px-2 py-1 rounded w-fit">{entry.tag}</span>
 </div>
 <div className="p-6 md:w-3/4">
 <h3 className="font-bold text-[var(--foreground)] mb-4 border-b border-[var(--border)] pb-2">Λίστα Αλλαγών</h3>
 <ul className="space-y-3">
 {entry.changes.map((change, cIdx) => (
 <li key={cIdx} className="flex items-start gap-3 text-sm text-[var(--text-secondary)]">
 {change.type === 'NEW' && <span className="bg-[var(--success-light)] text-[var(--success)] text-[10px] font-extrabold px-1.5 py-0.5 rounded shadow-sm shrink-0">NEW</span>}
 ...
 {change.type === 'FIX' && <span className="bg-slate-100 text-[var(--text-secondary)] text-[10px] font-extrabold px-1.5 py-0.5 rounded shadow-sm shrink-0">FIX</span>}
