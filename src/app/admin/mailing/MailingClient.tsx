...
export default function MailingClient({ contacts, smtpConfigured }: MailingClientProps) {
 ...
 tab === 'labels'
 ? 'bg-[var(--surface)] (--surface)] text-[var(--foreground)] shadow-sm'
 ...
 <div className="flex items-start gap-3 p-4 bg-[var(--warning-light)] border border-amber-200 rounded-xl text-sm text-[var(--warning)]">
 ...
 {/* Toolbar */}
 {tab === 'labels' && (
 <div className="bg-[var(--surface)] border border-border p-4 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
 <div className="flex items-center gap-4 w-full max-w-md">
 <div className="relative flex-1">
 <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)]"/>
 <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Αναζήτηση Ενορίτη..."className="pl-9 bg-[var(--background)]"/>
 </div>
 </div>
 <div className="flex items-center gap-4 w-full md:w-auto">
 <div className="text-sm font-semibold text-[var(--text-muted)] bg-slate-100 px-3 py-2 rounded-lg">
 ...
 <div className="text-sm font-semibold text-[var(--text-muted)] bg-slate-100 px-3 py-2 rounded-lg">
 Επιλεγμένοι: <span className="text-[var(--brand)] font-bold">{selectedIds.length}</span>
 </div>
 </div>
)}
 ...
 className="bg-[var(--brand)] hover:bg-[var(--brand-dark)] text-white gap-2"
 >
 {isSending ? (
 ...
 <tr><td colSpan={5} className="p-12 text-center text-[var(--text-muted)]">Δεν βρέθηκαν επαφές.</td></tr>
 ...
 <div className="text-xs text-[var(--text-muted)]">Τ.Κ. {contact.postalCode || '---'}</div>
 </td>
)}
 {tab === 'email' && (
 <td className="p-4 text-[var(--text-secondary)]">
 {contact.email || <span className="text-[var(--text-muted)] italic text-xs">Χωρίς email</span>}
 </td>
)}
 <td className="p-4 text-[var(--text-muted)]">{contact.phone || contact.mobile || '-'}</td>
 <td className="p-4 text-center">
 {tab === 'email' && !hasEmail ? (
 <span className="bg-slate-100 text-[var(--text-muted)] text-xs px-2 py-1 rounded-full font-semibold flex items-center justify-center gap-1 w-fit mx-auto">
