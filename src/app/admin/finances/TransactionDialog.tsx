...
export default function TransactionDialog({ categories }: { categories: any[] }) {
 ...
 if (!date) return alert('Επιλέξτε Ημερομηνία')
 ...
 <Button onClick={() => { setType('INCOME'); setIsOpen(true); }} className="btn btn-success gap-2 font-bold shadow-lg shadow-emerald-500/30 transition-transform active:scale-95">
 <ArrowUpRight className="w-4 h-4"/> Νέο Έσοδο
 </Button>
 <Button onClick={() => { setType('EXPENSE'); setIsOpen(true); }} className="btn btn-danger gap-2 font-bold shadow-lg shadow-rose-500/30 transition-transform active:scale-95">
 <ArrowDownRight className="w-4 h-4"/> Νέο Έξοδο
 </Button>
 </div>

 <Dialog open={isOpen} onOpenChange={setIsOpen}>
 <DialogContent showCloseButton={false} className="p-0 border-none bg-transparent shadow-none sm:max-w-xl gap-0 outline-none">
 <DialogTitle className="sr-only">Νέα Συναλλαγή</DialogTitle>
 <div className="card w-full max-w-xl shadow-2xl border border-border/50 overflow-hidden relative">
 ...
 <button type="button" onClick={() => setIsOpen(false)} className="text-[var(--text-muted)] hover:text-slate-700 transition-colors bg-slate-100 p-2 rounded-full shadow-sm">
 ...
 <Label className="font-semibold text-slate-700 flex items-center gap-1.5"><CalIcon className="w-4 h-4"/> Ημερομηνία</Label>
 <Input type="date" value={date} onChange={e => setDate(e.target.value)} required className="h-12 rounded-xl border-[var(--border)] border-2"/>
 </div>
 ...
 <Button type="submit" disabled={loading || !categoryId || !date} className={`h-12 px-8 rounded-xl font-bold shadow-lg text-white transition-all transform active:scale-95 ${type === 'INCOME' ? 'btn btn-success shadow-emerald-500/30' : 'btn btn-danger shadow-rose-500/30'}`}>
