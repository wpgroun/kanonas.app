...
export default function TransactionDialog({ categories }: { categories: any[] }) {
 ...
 <div className="bg-white w-full max-w-xl rounded-2xl shadow-2xl border border-border/50 overflow-hidden animate-in zoom-in-95 duration-300 relative">
 ...
 <h2 className={`text-2xl font-extrabold flex items-center gap-3 tracking-tight ${type === 'INCOME' ? 'text-emerald-700 ' : 'text-rose-700 '}`}>
 ...
 <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-slate-700 :text-slate-200 transition-colors bg-slate-100 p-2 rounded-full shadow-sm">
 ...
 className="w-full flex h-12 items-center justify-between rounded-xl border-2 border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 shadow-sm transition-colors focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20"
 >
 ...
 <Input value={purpose} onChange={e => setPurpose(e.target.value)} placeholder="π.χ. Αγορά Γραφικής Ύλης, Δωρεά Φιλοπτώχου"className="h-12 rounded-xl border-slate-200 border-2 focus-visible:ring-indigo-500/20"/>
 </div>

 <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
 <div className="space-y-2">
 <Label className="font-semibold text-slate-700">{type === 'INCOME' ? 'Δωρητής / Ενορίτης' : 'Προμηθευτής / Εταιρεία'}</Label>
 <Input value={donorOrVendor} onChange={e => setDonorOrVendor(e.target.value)} placeholder={type === 'INCOME' ?"Ανώνυμος...":"Πλαίσιο A.E."} className="h-12 rounded-xl border-slate-200 border-2"/>
 </div>
 <div className="space-y-2">
 <Label className="font-semibold text-slate-700">Αρ. Παραστατικού</Label>
 <Input value={receiptNumber} onChange={e => setReceiptNumber(e.target.value)} placeholder="π.χ. ΤΔ-1234"className="h-12 rounded-xl border-slate-200 border-2"/>
 </div>
 </div>
