...
export default function TransactionDialog({ categories }: { categories: any[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-md flex flex-col items-center justify-center z-[999] p-4 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-2xl shadow-2xl border border-border/50 overflow-hidden animate-in zoom-in-95 duration-300 relative">
        ...
        <div className="relative p-6 px-7 flex justify-between items-center border-b border-border/20 z-10">
          <h2 className={`text-2xl font-extrabold flex items-center gap-3 tracking-tight ${type === 'INCOME' ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'}`}>
          ...
          <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors bg-slate-100 dark:bg-slate-800 p-2 rounded-full shadow-sm">
            <X className="w-5 h-5"/>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-7 space-y-6 relative z-10">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner">
             <Label className="text-muted-foreground font-bold mb-2 block uppercase tracking-wider text-xs">Συνολικό Ποσό</Label>
             <div className="relative flex items-center">
               <span className={`absolute left-4 font-black text-3xl ${type === 'INCOME' ? 'text-emerald-500' : 'text-rose-500'}`}>€</span>
               <input 
                  type="number" 
                  step="0.01" 
                  value={amount} 
          ...
          <div className="space-y-2">
             <Label className="font-semibold text-slate-700 dark:text-slate-300">Λογαριασμός (Κατηγορία)</Label>
             {categories.length === 0 ? (
                <div className="text-sm text-destructive font-semibold p-3 bg-red-50 dark:bg-red-500/10 rounded-xl">Δεν έχετε δημιουργήσει κατηγορίες στο Λογιστικό Σχέδιο!</div>
             ) : (
                <select 
                  required value={categoryId} onChange={e => setCategoryId(e.target.value)}
                  className="w-full flex h-12 items-center justify-between rounded-xl border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-2 text-sm text-slate-800 dark:text-slate-100 shadow-sm transition-colors focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20"
                >
          ...
          <div className="space-y-2">
             <Label className="font-semibold text-slate-700 dark:text-slate-300">Αιτιολογία & Σημειώσεις (Προαιρετικό)</Label>
             <Input value={purpose} onChange={e => setPurpose(e.target.value)} placeholder="π.χ. Αγορά Γραφικής Ύλης, Δωρεά Φιλοπτώχου" className="h-12 rounded-xl border-slate-200 border-2 dark:border-slate-800 focus-visible:ring-indigo-500/20" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
             <div className="space-y-2">
                <Label className="font-semibold text-slate-700 dark:text-slate-300">{type === 'INCOME' ? 'Δωρητής / Ενορίτης' : 'Προμηθευτής / Εταιρεία'}</Label>
                <Input value={donorOrVendor} onChange={e => setDonorOrVendor(e.target.value)} placeholder={type === 'INCOME' ? "Ανώνυμος..." : "Πλαίσιο A.E."} className="h-12 rounded-xl border-slate-200 border-2 dark:border-slate-800" />
             </div>
             <div className="space-y-2">
                <Label className="font-semibold text-slate-700 dark:text-slate-300">Αρ. Παραστατικού</Label>
                <Input value={receiptNumber} onChange={e => setReceiptNumber(e.target.value)} placeholder="π.χ. ΤΔ-1234" className="h-12 rounded-xl border-slate-200 border-2 dark:border-slate-800" />
             </div>
          </div>

          <div className="pt-6 mt-6 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
             <Button type="button" variant="ghost" onClick={() => setIsOpen(false)} className="h-12 px-6 rounded-xl font-semibold text-slate-500 hover:text-slate-800">Ακύρωση</Button>
