...
export default function StaffClient({ initialStaff, initialBoard }: any) {
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {laymen.length === 0 ? (
 <div className="col-span-full py-12 text-center text-gray-500 border border-dashed rounded-xl">Δεν υπάρχει καταχωρημένο λαϊκό προσωπικό.</div>
) : laymen.map((s: any) => (
 <Card key={s.id} className="p-4 flex flex-col gap-3 border border-border">
 <div className="flex items-center gap-3">
 ...
 <p className="text-sm text-gray-500">{s.role?.name || 'Απλό Προσωπικό'}</p>
 </div>
 </div>
 ...
 <tr><td colSpan={3} className="p-8 text-center text-gray-500">Η Επιτροπή δεν έχει συγκροτηθεί.</td></tr>
) : initialBoard.map((m: any) => (
 <tr key={m.id} className="hover:bg-gray-50/50 :bg-gray-800/50">
 <td className="p-4 font-medium">{m.firstName} {m.lastName}</td>
 <td className="p-4">
 <span className={`px-2 py-1 rounded text-xs font-semibold ${m.role==='Πρόεδρος'?'bg-emerald-100 text-emerald-800':m.role==='Ταμίας'?'bg-amber-100 text-amber-800':'bg-blue-100 text-blue-800'}`}>
 {m.role}
 </span>
 </td>
