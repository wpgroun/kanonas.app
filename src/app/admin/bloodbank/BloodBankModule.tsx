...
export default function BloodBankModule({ dashboard, donors, drives }: any) {
 ...
 className={`rounded-lg gap-1.5 text-xs shadow-none ${activeView === tab.id ? 'badge badge-danger hover:bg-red-100' : ''}`}
...
function DonorsView({ donors }: { donors: any[] }) {
 ...
 <DialogTrigger asChild><Button className="gap-2 btn btn-danger"><Plus className="w-4 h-4"/>Νέος Δότης</Button></DialogTrigger>
 <DialogContent>
 ...
 <span className="px-2 py-1 badge badge-danger rounded-lg font-bold text-lg">{d.bloodType}</span>
 </td>
 <td className="p-3 text-center font-bold text-muted-foreground">{d.totalDonations}</td>
 <td className="p-3">
 {elig.status === 'eligible' && <Badge variant="outline" className="border-emerald-200 badge badge-success gap-1"><CheckCircle2 className="w-3 h-3"/>Επιλέξιμος</Badge>}
 {elig.status === 'soon' && <Badge variant="outline" className="border-amber-200 badge badge-warning gap-1"><AlertCircle className="w-3 h-3"/>Σε {elig.daysUntil} ημέρες</Badge>}
 {elig.status === 'ineligible' && <div><Badge variant="outline" className="border-red-200 badge badge-danger gap-1"><Droplet className="w-3 h-3"/>Αναμονή ({elig.daysUntil}ημ.)</Badge><div className="text-[10px] text-muted-foreground mt-1">Από: {new Date(d.lastDonation).toLocaleDateString('el-GR')}</div></div>}
 </td>
...
function DonorForm({ onDone }: { onDone: () => void }) {
 ...
 <div><Label>Επώνυμο *</Label><Input value={lastName} onChange={e => setLastName(e.target.value)} className="bg-white border-2 border-border" required/></div>
 <div><Label>Όνομα *</Label><Input value={firstName} onChange={e => setFirstName(e.target.value)} className="bg-white border-2 border-border" required/></div>
 </div>
 <div className="grid grid-cols-2 gap-3">
 <div><Label>Τηλέφωνο</Label><Input type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="bg-white border-2 border-border"/></div>
 <div>
 <Label>Ομάδα Αίματος *</Label>
 <select value={bloodType} onChange={e => setBloodType(e.target.value)} className="w-full p-2 border-2 border-border rounded-md text-sm bg-white " required>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div><Label>Ημ/νία Γέννησης</Label><Input type="date" value={dob} onChange={e => setDob(e.target.value)} className="bg-white border-2 border-border"/></div>
 <div>
 <Label>Φύλο</Label>
 <select value={gender} onChange={e => setGender(e.target.value)} className="w-full p-2 border-2 border-border rounded-md text-sm bg-white ">
 <option value="">— Επιλογή —</option><option value="M">Άνδρας</option><option value="F">Γυναίκα</option>
 </select>
 </div>
 </div>
 <Button type="submit" disabled={loading} className="w-full btn btn-danger">Αποθήκευση</Button>
 </form>
 )
}
...
function LogDonationDialog({ donor, onDone }: { donor: any, onDone: () => void }) {
 ...
 <div className="grid grid-cols-2 gap-3">
 <div><Label>Ημερομηνία *</Label><Input type="date" name="date" required defaultValue={new Date().toISOString().split('T')[0]} className="bg-white border-2 border-border"/></div>
 <div><Label>Νοσοκομείο / Κέντρο</Label><Input name="hospital" className="bg-white border-2 border-border"/></div>
 </div>
 <div><Label>Αιμοσφαιρίνη (Hgb g/dL - προαιρετικό)</Label><Input type="number" step="0.1" name="hemo" className="bg-white border-2 border-border"/></div>
 <Button type="submit" disabled={loading} className="w-full btn btn-danger">Καταχώρηση Ευχαριστήριου</Button>
 </form>
 </DialogContent>
 </Dialog>
 )
...
function DrivesView({ drives }: { drives: any[] }) {
 ...
 <DialogTrigger asChild><Button className="btn btn-danger gap-2"><Plus className="w-4 h-4"/>Νέα Εκστρατεία</Button></DialogTrigger>
 ...
 <div><Label>Τίτλος *</Label><Input name="name" required placeholder="π.χ. Εαρινή Αιμοδοσία Αγ. Δημητρίου" className="bg-white border-2 border-border"/></div>
 <div className="grid grid-cols-2 gap-3">
 <div><Label>Ημερομηνία *</Label><Input type="date" name="date" required className="bg-white border-2 border-border"/></div>
 <div><Label>Τοποθεσία</Label><Input name="location" placeholder="Αίθουσα Ι. Ναού" className="bg-white border-2 border-border"/></div>
 </div>
 <div className="grid grid-cols-2 gap-3">
 <div><Label>Συνεργαζόμενο Νοσοκομείο</Label><Input name="hospital" placeholder="π.χ. Γενικό Κρατικό" className="bg-white border-2 border-border"/></div>
 <div><Label>Στόχος (Φιάλες)</Label><Input type="number" name="target" placeholder="π.χ. 50" className="bg-white border-2 border-border"/></div>
 </div>
 <Button type="submit" className="w-full btn btn-danger">Δημιουργία</Button>
 </form>
 </DialogContent>
 </Dialog>
 </div>
 ...
 <Badge variant={d.status === 'COMPLETED' ? 'outline' : 'primary'} className={d.status === 'PLANNED' ? 'badge badge-warning hover:bg-amber-100' : ''}>
...
function EmergencyView() {
 ...
 <Button onClick={handleSearch} disabled={loading} className="w-full btn btn-danger h-12 text-base shadow-lg shadow-red-500/20">
