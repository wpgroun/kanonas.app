...
export default function ProfileClient({ 
 ...
 <h2 className="text-lg font-bold text-[var(--foreground)] mb-4 border-b border-[var(--border)] pb-2">Στοιχεία Λογαριασμού</h2>
 
 <form onSubmit={handleProfileUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="col-span-full mb-2">
 <label className="block text-sm font-bold text-[var(--text-muted)] mb-1">Email <span className="text-xs font-normal">(Δεν αλλάζει)</span></label>
 <input type="email"value={user.email} disabled className="data-input w-full bg-[var(--background)] text-[var(--text-muted)] cursor-not-allowed"/>
 </div>
 ...
 <div>
 {profileError && <span className="text-[var(--danger)] text-sm font-medium">{profileError}</span>}
 ...
 {/* Αλλαγή Κωδικού */}
 <div className="bg-[var(--surface)] rounded-2xl border border-rose-100 p-6 shadow-sm relative overflow-hidden">
 <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--danger-light)] rounded-bl-full -z-10"/>
 
 <h2 className="text-lg font-bold text-[var(--foreground)] mb-4 border-b border-[var(--border)] pb-2 flex items-center gap-2">
 ...
 <div>
 {passError && <span className="text-[var(--danger)] text-sm font-medium">{passError}</span>}
 ...
 <div>
 <h2 className="text-lg font-bold text-[var(--foreground)]">Ενεργές Συνεδρίες (Sessions)</h2>
 <p className="text-sm text-[var(--text-muted)]">Οι συσκευές στις οποίες είστε συνδεδεμένοι.</p>
 </div>
 <button 
 onClick={async () => {
 ...
 className="bg-[var(--danger-light)] text-[var(--danger)] hover:bg-[var(--danger-light)] px-4 py-2 rounded-xl text-sm font-bold transition-colors"
 >
 Αποσύνδεση Παντού
 </button>
 </div>
 ...
 {sessions.map((s) => (
 <div key={s.id} className="flex justify-between items-center bg-[var(--background)] border border-[var(--border)] p-4 rounded-xl">
 <div>
 <div className="font-bold text-[var(--foreground)] text-sm flex items-center gap-2">
 {s.userAgent?.includes('Mobi') ? '📱 Mobile' : '💻 Desktop'} 
 <span className="text-xs text-[var(--text-muted)] font-normal">({s.userAgent})</span>
 </div>
 ...
 <span className="bg-[var(--success-light)] text-emerald-800 text-xs font-bold px-3 py-1 rounded-full">Αυτή η συσκευή</span>
)}
 </div>
))}
 </div>
