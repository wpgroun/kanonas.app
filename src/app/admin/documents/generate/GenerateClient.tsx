...
export default function GenerateClient({ templates, selectedTemplateId }: { templates: any[], selectedTemplateId?: string }) {
  ...
  const variables = useMemo(() => {
    if (!selected) return [];
    // From context (uploaded PDFs) — supports both old array format and new { format, vars } object format
    try {
      if (selected.context) {
        const parsed = JSON.parse(selected.context);
        if (Array.isArray(parsed)) return parsed;                              // old format
        if (parsed.vars && Array.isArray(parsed.vars)) return parsed.vars;    // new format
      }
    } catch {}
    // From HTML content
