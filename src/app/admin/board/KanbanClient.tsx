...
export default function KanbanClient({ initialTasks }: { initialTasks: any[] }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-8">
       ...
       {!isAdding ? (
          <button onClick={() => setIsAdding(true)} className="bg-white border border-dashed border-gray-300 rounded-xl p-4 flex items-center justify-center gap-2 text-gray-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 transition-all font-semibold shadow-sm w-full md:w-64">
             <Plus className="w-5 h-5"/> Νέα Εργασία
          </button>
       ) : (
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-xl max-w-md w-full animate-in fade-in slide-in-from-top-4">
             <h3 className="font-bold text-gray-800 mb-4">Προσθήκη Εργασίας</h3>
             <div className="space-y-3">
                ...
                <div className="flex gap-2 justify-end pt-2">
                   <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-md text-sm font-semibold">Ακύρωση</button>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start mt-2">
          {columns.map(col => {
             ...
             return (
                <div key={col.id} className={`${col.bg} p-4 rounded-xl border-t-4 ${col.color} border-l border-r border-b border-gray-200 min-h-[500px]`}>
                   <div className="flex items-center justify-between mb-4">
                      <h3 className="font-extrabold text-gray-700 uppercase tracking-wider text-sm">{col.title}</h3>
                      <span className="bg-white text-gray-500 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">{colTasks.length}</span>
                   </div>

                   <div className="flex flex-col gap-3">
                      {colTasks.length === 0 && <div className="text-center p-8 border-2 border-dashed border-gray-300 rounded-xl text-gray-400 text-sm">Καμία εργασία</div>}
                      
                      {colTasks.map(task => (
                         <div key={task.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 group hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-2">
                               <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${task.priority === 'HIGH' ? 'bg-red-100 text-red-700 w-fit flex items-center gap-1' : 'bg-gray-100 text-gray-500'}`}>
                            ...
                            <h4 className="font-bold text-gray-800 leading-tight mb-1">{task.title}</h4>
                            {task.description && <p className="text-xs text-gray-500 line-clamp-2 mt-1 mb-3">{task.description}</p>}
                            
                            <div className="flex items-center justify-between mt-4">
                               {task.assignee ? (
                                  <div className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md text-[10px] font-bold">
                               ...
                               <div className="flex items-center gap-1">
                                  {col.id !== 'TODO' && (
                                     <button onClick={() => moveTask(task.id, col.id === 'DONE' ? 'IN_PROGRESS' : 'TODO')} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors" title="Πίσω">
                                        <ArrowLeft className="w-4 h-4"/>
                                     </button>
                                  )}
                                  {col.id !== 'DONE' && (
                                     <button onClick={() => moveTask(task.id, col.id === 'TODO' ? 'IN_PROGRESS' : 'DONE')} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-blue-600 transition-colors" title="Μπροστά">
