...
export default function ParishionersClient({ parishioners }: { parishioners: any[] }) {
 ...
 {filtered.map((p: any) => (
 <Card key={p.id} className="card hover:shadow-md hover:border-primary/30 transition-all group overflow-hidden flex flex-col">
 ...
 <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium badge badge-success">
 Ενεργός
 </span>
 ) : p.status === 'inactive' ? (
 <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium badge bg-slate-100 text-slate-700 border border-slate-200">
 Ανενεργός
 </span>
 ) : p.status === 'deceased' ? (
