...
export async function GET(req: NextRequest) {
 ...
 logger.error(`[Cron] Failed to send reminder for token ${token.id}:`, emailError);
 results.push({ id: token.id, status: 'error', reason: String(emailError) });
 }
 }

 logger.info(`[Cron /api/cron/reminders] Processed ${upcomingTokens.length} tokens:`, results);
 ...
 logger.error('[Cron] Fatal error in reminders cron:', error);
