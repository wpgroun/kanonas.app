...
export async function createSacramentRequest(formData: {
 ...
 logger.error("Σφάλμα αποθήκευσης:", error)
...
export async function linkPersonToSacrament(tokenId: string, parishionerId: string, role: string) {
 ...
 logger.error("Σφάλμα σύνδεσης προσώπου:", error)
...
export async function removePersonFromSacrament(ceremonyPersonId: string) {
  ...
  logger.error("Σφάλμα διαγραφής προσώπου:", e)
...
export async function markTokenAsDocsGenerated(tokenId: string, assignedPriest: string, bookNumber?: string, protocolNumber?: string) {
 ...
 logger.error("Auto enrol or markToken error", e);
...
export async function savePublicTokenAnswers(tokenStr: string, answersStr: string, personsArr?: {role: string, firstName: string, lastName: string, fathersName?: string, mothersName?: string}[]) {
 ...
 logger.error("Σφάλμα αποθήκευσης questionnaire:", error)
 return { success: false, error:"Αποτυχία υποβολής."}
 }
}

// ── Admin: update civil-registry / ceremony meta fields directly ─────────────
export async function updateCeremonyMetaByAdmin(
  ...
  try {
    ...
    // Read existing dataJson, merge new fields on top
    ...
    // Merge — overwrite only provided (non-empty) fields
    ...
    logger.error('updateCeremonyMetaByAdmin error:', e);
