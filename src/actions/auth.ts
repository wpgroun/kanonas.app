...
export async function forgotPasswordAction(email: string) {
 ...
 logger.info(`[DEV] Password reset link for ${email}: ${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/reset-password?token=${token}`)
 } else {
 logger.warn(`[WARN] Password reset requested for ${email} but SMTP is not configured.`);
 }
...
export async function resetPasswordAction(token: string, newPasswordPlain: string) {
 ...
 logger.error('[resetPasswordAction] Error:', e);
